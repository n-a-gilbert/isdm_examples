...
data_camera <- list(y = data$y, nsurveys = data$nsurveys, 
                    num = data$num, adj = data$adj, 
                    weights = data$weights, forest = data$forest, 
                    imperv = data$imperv)

constants_camera <- list(ncell = constants$ncell, 
                         cell = constants$cell, 
                         nsite = constants$nsite, 
                         k = constants$k)
...
# model should run in 5-10 minutes with these settings
...
# create model
model <- nimble::nimbleModel(code = camera_only, 
                             data = data_camera, 
                             constants = constants_camera, 
                             inits = inits())
